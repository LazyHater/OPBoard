...
// others
float pos = 90.0f;
#define RESISTOR_CONSTANT (5.316f)
#define V_REF (1.1f)
...
float calc_accel() {
  float y_val = (float) (nunchuk_joystickY_raw() - 128); // scale to -255.0f to 255.0f
  y_val /= 127.0f; // scale to -1.0 to 1.0f
  if(nunchuk_buttonZ()) y_val *= 3.0f;
  pos += y_val;
  pos = constrain(pos, 0.0f, 180.0f);  
  if((!nunchuk_buttonZ())&&(!nunchuk_buttonC())) pos = 90.0f;
...
void loop(void) {
    ...
    if ( radio.available() )
    {
      ...
      while (!done)
      {
        ...
        done = radio.read( &got_val, sizeof(uint8_t));
